...
function filterEventsByDateRange(
  ...
  console.log('After filtering:', { filteredCount: filteredEvents.length, sampleDates: filteredEvents.slice(0, 5).map(e => e.date) })
...
function transformScheduleData(
  ...
  return sheetData.content.map((row: any, index: number) => {
    ...
    const personFields = extractPersonFields(row, sheetData.type as Exclude<GoogleSheetTypes, 'directory'>)
    
    // User highlighting - check if current user is involved in this event
    const userHighlight = currentUser ? 
      personFields.some(name => 
        name && name.toLowerCase().includes(currentUser.split('@')[0].toLowerCase())
      ) : false
    ...
    const torontoEventDate = new Date(eventDate.toLocaleString('en-US', { timeZone: 'America/Toronto' }))
    
    const isToday = torontoEventDate.toDateString() === torontoToday.toDateString()
    const isUpcoming = torontoEventDate >= torontoToday && torontoEventDate <= new Date(torontoToday.getTime() + 7 * 24 * 60 * 60 * 1000)
    const isNextEvent = isToday || (isUpcoming && index === 0)
    
    // Conflict detection - check if user has multiple responsibilities on same date
    const hasConflict = currentUser && allScheduleData ? 
      checkForScheduleConflicts(formattedDate, currentUser, allScheduleData) : false
    ...
    addScheduleSpecificFields(event, row, sheetData.type as Exclude<GoogleSheetTypes, 'directory'>)
...
function extractPersonFields(row: any, scheduleType: Exclude<GoogleSheetTypes, 'directory'>): string[] {
  switch (scheduleType) {
    case 'memorial':
      return [
        row.Preside || '',
...
function addScheduleSpecificFields(event: any, row: any, scheduleType: Exclude<GoogleSheetTypes, 'directory'>): void {
  switch (scheduleType) {
    case 'memorial':
...
export async function GET(request: NextRequest) {
  try {
    ...
    const requestedTypes = searchParams.get('types')?.split(',') as Exclude<GoogleSheetTypes, 'directory'>[] || 
      ['memorial']
    ...
    Object.keys(transformedData).forEach(type => {
      transformedData[type] = transformedData[type].map(event => ({
        ...
        hasConflict: currentUser ? 
          checkForScheduleConflicts(event.date, currentUser, allScheduleData) : false
      }))
    })
    ...
    if (sortOrder === 'desc') {
      ...
      if (returnedAnyEvents) {
        ...
        if (allReturnedEvents.length > 0) {
          const earliestReturnedDate = allReturnedEvents.reduce((earliest, event) => 
          ...
          hasMoreHistoricalData = requestedTypes.some(type => 
            transformedData[type] && transformedData[type].some(event => event.date < earliestReturnedDate)
          )
        }
      }
    } else {
      ...
      hasMoreHistoricalData = requestedTypes.some(type => 
        transformedData[type] && transformedData[type].some(event => event.date < today)
      )
    }
