  ...
  try {
    ...
    const dateAnalysis = dynamoData.content.slice(0, 20).map((entry: any, index: number) => {
      ...
      const torontoDate = new Date(
        parsedDate.toLocaleString('en-US', { timeZone: 'America/Toronto' })
      )
      ...
      return {
        ...
        otherFields: Object.keys(entry)
          .filter((key) => !['Date', 'date'].includes(key))
          .slice(0, 3),
      }
    })
    ...
    const summary = {
      ...
      dayDistribution: dateAnalysis.reduce(
        (acc, d) => {
