  ...
  try {
    ...
    const comparison = {
      ...
      dynamoDbData: dynamoData ? {
        ...
        sampleEntries: dynamoData.content?.slice(0, 3).map((entry: any) => ({
      ...
      googleSheetsData: googleData ? {
        ...
        sampleEntries: googleData.content?.slice(0, 3).map((entry: any) => ({
          rawDate: entry.Date || entry.date,
          dateType: typeof (entry.Date || entry.date),
          parsedDate: new Date(entry.Date || entry.date),
          torontoDate: new Date(entry.Date || entry.date).toLocaleString('en-US', { 
      ...
      analysis: {
        ...
        dataMismatch: dynamoData && googleData ? 
          dynamoData.content?.length !== googleData.content?.length : null,
        firstEntryComparison: dynamoData?.content?.[0] && googleData?.content?.[0] ? {
          ...
          datesMatch: (dynamoData.content[0].Date || dynamoData.content[0].date) === 
