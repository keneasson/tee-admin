...
async function handleMergeContacts(body: any, session: any) {
  ...
  if (sourceIsSESOnly || targetIsSESOnly) {
    ...
    try {
      ...
      if (sesContact && sesContact.Contacts && sesContact.Contacts.length > 0) {
        ...
        isUnsubscribed = !contact.TopicPreferences ||
    ...
    const directoryRecord = await docClient.send(new GetCommand({
    ...
    if (!directoryRecord.Item) {
      return NextResponse.json(
        { error: 'Directory record not found' },
        { status: 404 }
      )
    }
    ...
    const existingEmailsQuery = await docClient.send(new QueryCommand({
      TableName: 'tee-schedules',
    ...
    const activeEmailCount = existingEmails.filter(item => !item.status || item.status === 'active').length
  ...
  if (!sourceRecord.Item || !targetRecord.Item) {
    return NextResponse.json(
      { error: 'Source or target record not found' },
      { status: 404 }
    )
  }
...
async function handleMigrateEmail(body: any, session: any) {
  ...
  try {
    ...
    const unsubscribedLists = Object.keys(EmailListTypes).reduce((acc, key) => {
    ...
    await docClient.send(new UpdateCommand({
      ...
      UpdateExpression: 'SET email = :newEmail, #status = :status, lastUpdated = :timestamp, migratedFrom = :oldEmail, migratedBy = :admin',
    ...
    await logAuditEvent({
      ...
      oldEmail,
      newEmail,
      subscriptionsCopied: Object.keys(oldSubscriptions).filter(k => oldSubscriptions[k as EmailListTypeKeys]),
      timestamp: new Date().toISOString()
    })
...
async function handleReorderEmails(body: any, session: any) {
  ...
  if (emails.length > 2) {
    return NextResponse.json(
      { error: 'Maximum of 2 emails per person' },
      { status: 400 }
    )
  }
