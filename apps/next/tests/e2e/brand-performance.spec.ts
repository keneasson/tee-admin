...
test.describe('Brand System Visual Regression', () => {
  test('should maintain consistent color palette layout', async ({ page }) => {
    ...
    const colorCards = await page.locator('[data-testid="color-card"], .color-card, *:has-text("primary"):has-text("#")').count()
  ...
  test('should maintain consistent component showcase layout', async ({ page }) => {
    ...
    const componentShowcases = await page.locator('*:has-text("Form Input"), *:has-text("Password Input"), *:has-text("Navigation Button")').count()
