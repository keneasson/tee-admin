...
- **CRITICAL — SES contact-list / topic edits are DESTRUCTIVE. Back up FIRST.** `aws sesv2 update-contact-list` (topic add/remove) **wipes every contact's `TopicPreferences`** (per-contact opt-ins/opt-outs) — a topic-list edit on 2026-07-04 zeroed the whole mailing list and there was no backup (SES was the source of truth; PersonRecords held no `sesTopicPreferences`). Before ANY `sesv2 update-contact-list` / `create-contact` / `delete-contact` / topic mutation: **export a full backup** (all contacts with `TopicPreferences` + `AttributesData` via `get-contact`, the topic list, and the account suppression list) and save it to the **private, encrypted `tee-admin-backups` S3 bucket** (`backups/` prefix) — **NEVER commit member emails / PII to the repo** (it's PII on GitHub; one visibility flip leaks it — the legacy `apps/next/data/contact-subscribers*.json` + `subscribed_email_audience_export_*.csv` are a pre-existing exposure to remove). The account-level **suppression list** (bounces/complaints — the "master scrub") is a SEPARATE system and survives topic edits; contact-list `TopicPreferences` do NOT. Recovery source of truth is the **directory**, not the stale `contact-subscribers*.json` (2024).
